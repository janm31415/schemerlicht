...
namespace
  ...
  void print_ptr(uint64_t rax2, std::ostream& out, std::shared_ptr<environment<environment_entry>> env, const repl_data& rd, const context* p_ctxt)
    {
...
void print_last_global_variable_used(std::ostream& out, std::shared_ptr<environment<environment_entry>> env, const repl_data& rd, const context* p_ctxt)
  {
  if (!p_ctxt)
    return;
  if (!env.get())
  ...
  for (int stack_item = SCHEME_VARIABLE_DEBUG_STACK_SIZE - 1; stack_item >= 0; --stack_item)
    ...
    if (env->find_if(res, [&](const std::pair<std::string, environment_entry>& v) { return v.second.pos == pos; }))
      {
...
void scheme_runtime(uint64_t rax, std::ostream& out, std::shared_ptr<environment<environment_entry>> env, const repl_data& rd, const context* p_ctxt)
  {
