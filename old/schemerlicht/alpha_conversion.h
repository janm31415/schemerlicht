...
struct alpha_conversion_data
  {
  COMPILER_SCHEMERLICHT_API alpha_conversion_data() : forward_declaration(false) {}
  COMPILER_SCHEMERLICHT_API alpha_conversion_data(const std::string& str) : forward_declaration(false), name(str) {}
  std::string name;
...
*/
COMPILER_SCHEMERLICHT_API void alpha_conversion(Program& prog, uint64_t& alpha_conversion_index, std::shared_ptr<environment<alpha_conversion_data>>& env, bool modify_names = true);
