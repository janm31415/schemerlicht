...
;; [JanM]
;; necessary imports (import is defined in core/modules.scm)
;; imports are not at the top of this file, as there is a test later on that expects
;; (define cur-section ...) as first line.
(import 'values)
(import 'dynamic-wind)
(import 'io)
...
(begin (begin (begin)
	      ...
	      (begin))
       (begin)
	   (define add3-bis (lambda (y) (+ y 3))) ; added by [JanM]: skiwi implementation remembers usage of + in definition of add3, so we make a new add3 to test the change of def of +
       (begin (begin (begin) (test '(3 6) add3-bis 6)	   
		     (begin))))
(set! + old-+)
(test 100 (lambda (x) (+ x 90)) 10) ; added by [JanM]: just testing whether + works again as normal
(test 9 add3 6)
(begin)
(begin (begin))
...
;      (string=? (symbol->string 'a) (symbol->string 'A))) ; r6rs is case sensitive, so this test is replaced
(test #f 'standard-case
      (string=? (symbol->string 'a) (symbol->string 'A)))	  
;(test #t 'standard-case
;      (or (string=? (symbol->string 'a) "A")
;	  (string=? (symbol->string 'A) "a"))) ; r6rs is case sensitive, so this test is replaced
(test #f 'standard-case
      (or (string=? (symbol->string 'a) "A")
...
;(test (string-standard-case "flying-fish") symbol->string 'flying-fish) ; r6rs is case sensitive, so this test is replaced
(test "flying-fish" symbol->string 'flying-fish)
;(test (string-standard-case "martin") symbol->string 'Martin) ; r6rs is case sensitive, so this test is replaced
(test "Martin" symbol->string 'Martin) 
(test "Malvina" symbol->string (string->symbol "Malvina"))
;(test #t 'standard-case (eq? 'a 'A)) ; r6rs is case sensitive, so this test is replaced
...
;(test #t eq? 'mISSISSIppi 'mississippi) ; r6rs is case sensitive, so this test is replaced
(test #f eq? 'mISSISSIppi 'mississippi)
;(test #f 'string->symbol (eq? 'bitBlt (string->symbol "bitBlt"))) ; r6rs is case sensitive, so this test is replaced
...
(test -1 expt -1 255)
;(test 1 expt -1 -256)  ; [JanM] exp is out of bounds in skiwi
;(test -1 expt -1 -255)  ; [JanM] exp is out of bounds in skiwi
(test 1 expt 256 0)
(test 1 expt -256 0)
(test 256 expt 256 1)
(test -256 expt -256 1)
...
(test 4 * 4)
(test 1 *)
;(test 1 / 1)  ; [JanM] next operations return a float in skiwi
;(test -1 / -1)
;(test 2 / 6 3)
;(test -3 / 6 -2)
;(test -3 / -6 2)
...
(define (test-inexact)
  ...
  #|
  ;[JanM]: tests removed, as 0.0 and -0.0 are not the same
  (cond ((= f0.0 (- f0.0))
	 (test #t eqv? f0.0 (- f0.0))
...
(define (test-bignum)
  ...
  (define b3-3 (string->number "3333333333333333")) ; [JanM] made number smaller to fit in skiwi's fixnum range
  (define b3-2 (string->number "3333333333333332")) ; [JanM] made number smaller to fit in skiwi's fixnum range
  (define b3-0 (string->number "3333333333333330")) ; [JanM] made number smaller to fit in skiwi's fixnum range
  (define b1-1 (string->number "1111111111111111")) ; [JanM] made number smaller to fit in skiwi's fixnum range
  ...
  ;(test b3-3 lcm b3-3 b1-1) ; [JanM] test removed, due to overflow (no bignums available)
  ...
  (SECTION 6 5 8)
  ;(test "281474976710655325431" number->string (string->number "281474976710655325431"))
  (test "2814749767106553254" number->string (string->number "2814749767106553254")) ; [JanM] made number smaller to fit skiwi's our fixnum range
  (report-errs))

(define (test-numeric-predicates)
  ;(let* ((big-ex (expt 2 150))
  (let* ((big-ex (expt 2 31)) ; [JanM] changed exp to 31, because no bignum support
...
(test #t eqv? '#\  #\space) ; [JanM] skiwi is capital sensitive
(test #t eqv? #\space '#\space) ; [JanM] skiwi is capital sensitive
(test #t char? #\a)
(test #t char? #\()
(test #t char? #\space)
(test #t char? '#\newline)
