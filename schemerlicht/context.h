...
COMPILER_SCHEMERLICHT_API context create_context(uint64_t heap_size, uint64_t globals_stack, uint32_t local_stack, uint64_t scheme_stack);
COMPILER_SCHEMERLICHT_API void destroy_context(context& ctxt);
COMPILER_SCHEMERLICHT_API context clone_context(const context& ctxt);
