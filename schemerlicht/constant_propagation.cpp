...
namespace
  ...
  struct is_unmutable_variable_visitor : public VisitorBaseClass
    {
    std::map<std::string, bool> is_unmutable;
    ...
    bool VisitVariable(Expression& e)
      {
      Variable& v = std::get<Variable>(e);
      auto it = is_unmutable.find(v.name);
      if (it == is_unmutable.end())
  ...
  struct replace_variable_visitor : VisitorBaseClass
    {
    std::string var_name;
    Expression replace_by_this_expr;
  ...
  struct constant_propagation_visitor : public VisitorBaseClass
    {
    std::map<std::string, bool>* p_is_unmutable;
