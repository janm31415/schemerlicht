...
void schemerlicht_primitive_current_seconds(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  ra->type = schemerlicht_object_type_flonum;
  ra->value.fl = cast(schemerlicht_flonum, clock()) / cast(schemerlicht_flonum, CLOCKS_PER_SEC);  
  UNUSED(c);
  UNUSED(b);
  }
...
void schemerlicht_primitive_current_milliseconds(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  ra->type = schemerlicht_object_type_flonum;
  ra->value.fl = cast(schemerlicht_flonum, clock()) * 1000.0 / cast(schemerlicht_flonum, CLOCKS_PER_SEC);  
  UNUSED(c);
  UNUSED(b);
  }
...
void schemerlicht_primitive_log(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = log(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = log(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_sin(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = sin(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = sin(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_cos(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = cos(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = cos(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_tan(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = tan(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = tan(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_asin(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = asin(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = asin(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_acos(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = acos(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = acos(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_atan(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b == 1)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = atan(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = atan(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
...
void schemerlicht_primitive_sqrt(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = sqrt(cast(schemerlicht_flonum, arg->value.fx));
        break;
      case schemerlicht_object_type_flonum:
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = sqrt(arg->value.fl);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
  ...
  if (b > 0)
    ...
    if (schemerlicht_object_get_type(arg) == schemerlicht_object_type_fixnum || schemerlicht_object_get_type(arg) == schemerlicht_object_type_flonum)
      ...
      if (schemerlicht_object_get_type(arg) == schemerlicht_object_type_fixnum)
        ...
        schemerlicht_string_init(ctxt, &ra->value.s, buffer);
        }
      else if (schemerlicht_object_get_type(arg) == schemerlicht_object_type_flonum)
        {
        if (arg->value.fl != arg->value.fl)
          {
          ra->type = schemerlicht_object_type_string;
          schemerlicht_string_init(ctxt, &ra->value.s, "+nan.0");
          }
          ...
          {
          ra->type = schemerlicht_object_type_string;
          schemerlicht_string_init(ctxt, &ra->value.s, "-inf.0");
          }
        else
          {
          ra->type = schemerlicht_object_type_string;
...
void schemerlicht_primitive_string_number(schemerlicht_context* ctxt, int a, int b, int c)
  ...
  if (b > 0)
    ...
    if (schemerlicht_object_get_type(arg) == schemerlicht_object_type_string && arg->value.s.string_length > 0)
      ...
      char* endptr;
      ra->type = schemerlicht_object_type_fixnum;
      ra->value.fx = strtoll(arg->value.s.string_ptr, &endptr, base);
      uintptr_t dist = endptr - arg->value.s.string_ptr;
      if (dist < arg->value.s.string_length)
        {
        ra->type = schemerlicht_object_type_flonum;
        ra->value.fl = strtod(arg->value.s.string_ptr, &endptr);
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ...
        break;
      case schemerlicht_object_type_flonum:
        ra->type = arg->value.fl == arg->value.fl ? schemerlicht_object_type_false : schemerlicht_object_type_true;
        break;
      default:
        break;
      }
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ...
        break;
      case schemerlicht_object_type_flonum:
        ra->type = isfinite(arg->value.fl) ? schemerlicht_object_type_false : (arg->value.fl == arg->value.fl ? schemerlicht_object_type_true : schemerlicht_object_type_false);
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
  ...
  if (b > 0)
    ...
    switch (schemerlicht_object_get_type(arg))
      ...
      case schemerlicht_object_type_fixnum:
        ...
        break;
      case schemerlicht_object_type_flonum:
        ra->type = isfinite(arg->value.fl) ? schemerlicht_object_type_true : schemerlicht_object_type_false;
        break;
      default:
        ra->type = schemerlicht_object_type_undefined;
        break;
  ...
  if (b > 0)
    ...
    if (schemerlicht_object_get_type(ch) == schemerlicht_object_type_char)
      ...
      if (ch->value.ch >= 'a' && ch->value.ch <= 'z')
        ra->value.ch = ch->value.ch - ('a' - 'A');
      else
        ra->value.ch = ch->value.ch;
      }
    }
  }
