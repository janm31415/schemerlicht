...
namespace
  ...
  struct naive_scan_visitor : public VisitorBaseClass
    {
    bool PreVisitLet(Expression& e)
      {
      Let& obj = std::get<Let>(e);
    ...
    bool PreVisitLamda(Expression& e)
      {
      Lambda& obj = std::get<Lambda>(e);
      obj.live_ranges.clear();
  ...
  struct find_liveness_of_variable : public VisitorBaseClass
    {
    std::map<std::string, uint64_t> var_last_occurence_map;

    void PostVisitVariable(Expression& e)
      {
  ...
  struct linear_scan_visitor : public VisitorBaseClass
    {
